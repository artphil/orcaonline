...
public class NCMResourceTest {
	...
	public void putRecursos_Sucesso_InserirRecursosExistentes() throws Exception {
		...
		NCM ncm1 = new NCM("12345678", "descricao");
		Familia fam2 = new Familia("12345678 modificado", "descricao modificada", null);
		
		when(this.repository.save(ncm1)).thenReturn(ncm1);
		...
		given().body(json).contentType(ContentType.JSON).when().put("/ncms/{id}", ncm1.getId()).andReturn().then().statusCode(HttpStatus.OK.value());
	...
	public void postRecursos_Sucesso_InserirRecursosExistentes() throws Exception {
		...
		ObjectWriter ow = new ObjectMapper().writer().withDefaultPrettyPrinter();
		String json = ow.writeValueAsString(ncm1);
				
		json = json.replaceAll("\"" + "id" + "\"[ ]*:[^,}\\]]*[,]?", "");
				
		System.out.println(json);
		
		given().body(json).contentType(ContentType.JSON).when().post("/ncms").andReturn().then().statusCode(HttpStatus.OK.value());
	...
	public void getRecursos_Sucesso_BuscarRecursosExistentes() throws Exception {
		...
		NCM ncm1 = new NCM("12345678", "descricao1");
		NCM ncm2 = new NCM("12345679", "descricao2");
		NCM ncm3 = new NCM("12345671", "descricao3");
		NCM ncm4 = new NCM("12345672", "descricao4");
		
		ncm1.setId(Long.valueOf(1));
		ncm2.setId(Long.valueOf(2));
		ncm3.setId(Long.valueOf(3));
		ncm4.setId(Long.valueOf(4));
		
		when(this.repository.findAll()).thenReturn(Arrays.asList(ncm1, ncm2, ncm3, ncm4));
		
		given().accept(ContentType.JSON).when().get("/ncms").andReturn().then().statusCode(HttpStatus.OK.value())
	    .expect(jsonPath("$[*]", hasSize(4)))
	    .expect(jsonPath("$[0].id", is(1)))
	    .expect(jsonPath("$[0].numero", is("12345678")))
	    .expect(jsonPath("$[0].descricao", is("descricao1")))
	    .expect(jsonPath("$[1].id", is(2)))
	    .expect(jsonPath("$[1].numero", is("12345679")))
	    .expect(jsonPath("$[1].descricao", is("descricao2")))
	    .expect(jsonPath("$[2].id", is(3)))
	    .expect(jsonPath("$[2].numero", is("12345671")))
	    .expect(jsonPath("$[2].descricao", is("descricao3")))
	    .expect(jsonPath("$[3].id", is(4)))
	    .expect(jsonPath("$[3].numero", is("12345672")))
	...
	public void getRecursos_EmptyList_BuscarRecursosInexistentes() {
		...
		given().accept(ContentType.JSON).when().get("/ncms").then().statusCode(HttpStatus.OK.value())
	...
	public void deleteById_NestedServletException_BuscarUmRecursoInexistente() {
		...
		Throwable exception = assertThrows(ArgumentsAreDifferent.class, () -> {
			...
			given().contentType(ContentType.JSON).when().delete("/ncms/{id}", 140L).andReturn().then().statusCode(HttpStatus.NO_CONTENT.value());
	...
	public void deleteById_Sucesso_BuscarUmRecursoExistente() throws JsonProcessingException {
		...
		given().contentType(ContentType.JSON).when().delete("/ncms/{id}", 1L).andReturn().then().statusCode(HttpStatus.NO_CONTENT.value());
