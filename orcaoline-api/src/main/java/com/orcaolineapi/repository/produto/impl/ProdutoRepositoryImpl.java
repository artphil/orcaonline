...
public class ProdutoRepositoryImpl implements ProdutoRepositoryQuery{
	...
	private Predicate[] criarRestricoes(ProdutoFilter filter, CriteriaBuilder builder, Root<Produto> root) {
		...
		if(filter.getFamilia() != null) {
			predicates.add(builder.equal(root.get("gtin").get("brick").get("classe").get("familia").get("id") , filter.getFamilia()));
		}
		if(filter.getSegmento() != null) {
			predicates.add(builder.equal(root.get("gtin").get("brick").get("classe").get("familia").get("segmento").get("id") , filter.getSegmento()));
		}
