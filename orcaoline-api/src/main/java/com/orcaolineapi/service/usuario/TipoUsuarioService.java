...
public class TipoUsuarioService extends AbstractService<TipoUsuario> {
	...
	public List<PermisaoTipoUsuarioEdicao> getPermissaoTipoUsuarioEdicao(FilterTipoUsuarioPermissao filtro) {
		List<Permissao> permissoes = filtro.getModulo() == null ? permissaoRepository.findAll()
				: permissaoRepository.findByModulo(filtro.getModulo());
		...
		for (Permissao p : permissoes) {
			PermisaoTipoUsuarioEdicao pu = new PermisaoTipoUsuarioEdicao(p.getId(), p.getNome(), p.getDescricao(),
					p.getModulo(), false);
